'use strict';
var bank = angular.module('bank', ['ngResource', 'ngRoute', 'ngCookies', 'datatables']);
...
bank.config(['$routeProvider', '$locationProvider','$httpProvider', function ($routeProvider, $locationProvider,$httpProvider) {
	...
	$scope.login = function(){
		if($scope.username && $scope.password){
			//console.log('###Test1###');
			 var username = "jdoe";
			 var pass = "Bank@123";
				if($scope.username == username && $scope.password == pass){
						AuthenticationService.SetCredentials($scope.username, $scope.password);
						userid = $scope.username;
						$location.path('/home');
					} else{
...
bank.controller('NavbarCtrl', ['$scope','$location', function($scope, $location){
...
bank.controller('homecontroller', ['$scope','$location','$http', function($scope, $location, $http){
	$scope.userID = userid;
	$scope.countries = [];
	$scope.fromcountries = [];
	$scope.tocountries = [];
  ...
  //   });
	...
	$http({
		method: "POST",
		url: "https://customerprofileservice.mybluemix.net/customerProfile",
		header: {
			'Content-Type': "application/json",
			'Access-Control-Allow-Headers': "Content-Type",
			'Access-Control-Allow-Methods': 'GET, POST, OPTIONS',
		...
		$scope.account_bal=JSON.stringify(data);
	...
	//$http.post('https://customerprofileservice.mybluemix.net/customerProfile').success(function (data) {
		
		

		/*$scope.accItems = data.body;
...
/**
 * Directive of Header and Footer Template
 */
bank.directive("masterHeader", function() {
    return {
        templateUrl : "modules/header/header.html"
    };
});

bank.directive("masterFooter", function() {
    return {
